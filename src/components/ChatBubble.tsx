...
export default function ChatBubble({ role, content, mood = "idle", isTyping, userPhoto }: Props) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {isTyping ? (
          <span style={{ display: "flex", gap: 4, alignItems: "center", height: 20 }}>
            {[0,1,2].map(i => (
              <span key={i} style={{
                width: 6, height: 6, borderRadius: "50%", background: "#C8B89A",
      ...
      {!isDogu && (
        userPhoto ? (
          <img src={userPhoto} style={{ width:36, height:36, borderRadius:"50%", objectFit:"cover", flexShrink:0, border:"1.5px solid #7B6A4E" }} alt="あなた" />
        ) : (
          <div style={{ width:36, height:36, borderRadius:"50%", background:"#C8B89A", display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, flexShrink:0 }}>👤</div>
        )
      )}
    </div>
  );
