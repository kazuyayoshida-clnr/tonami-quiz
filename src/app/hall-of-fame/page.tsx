...
export default function Certificate() {
  ...
  useEffect(() => {
    ...
    const day = d.getDate();
    setToday(`${y}年${m}月${day}日`);
  }, []);

  useEffect(() => {
    if (certUrl) {
      setQrUrl(`https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(certUrl)}&color=3B1F0A&bgcolor=F5EFE0`);
    }
  }, [certUrl]);
      ...
      <div className="cert-card" style={{
        ...
        <div style={{
          ...
          <div style={{ textAlign:"right" }}>
            <p style={{ margin:0, fontSize:11, color:"#8B5E3C", letterSpacing:"0.1em" }}>
              砺波市埋蔵文化財センター
            </p>
            <p style={{ margin:"2px 0 0", fontSize:15, fontWeight:700, color:"#3B1F0A", letterSpacing:"0.15em" }}>
              ドーグちゃん
            </p>
          </div>
        ...
        <div style={{
          ...
          <p style={{ fontSize:10, color:"#A08060", margin:"8px 0 0" }}>
            スクリーンショットで保存できます
          </p>
        </div>
      </div>
